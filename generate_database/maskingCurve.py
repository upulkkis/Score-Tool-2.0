...
def maskingCurve (fftX, noteNumber):
    ...
    if len(peaks) == 0:
        print("SUBSTITUTE!")
        locs, _ = find_peaks(fftX, threshold=10)
        peaks = np.round(fftX[locs], 2)
    ...
    for i in range (len(TH[:,1])):
        ...
        for k in range (len(Peak_list[:,0])):
            j = Peak_list[k, INDEX]
            indexi = int(Map[int(j)])
            if indexi == 107:
                indexi=106
            zj = TH[indexi, BARK] #Critical band rate of the masker #Tässä error crotaleksen kanssa 1218 IndexError: index 106 is out of bounds for axis 0 with size 106
    ...
    return masking_freq, masking_threshold, [locs, peaks]
